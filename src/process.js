...
$(function() {
    ...
    function onClick(elem) {
        ...
        if (next === current) { return false; } else {
            allow = false;
            setTimeout(function() { allow = true; }, time);
        }
    ...
    $('.process-slidePointer').on('click', function(e) {
        e.preventDefault();
        onClick($(this))
    });
    ...
    function checkCoords() {
        ...
        for (var k = 0; k < coordSize; k++) {
            if (t >= coordinates[k]) { $('.pipe:eq(' + k + ')').addClass('active'); } else { $('.pipe:eq(' + k + ')').removeClass('active'); }
        }
    };
    ...
    function checkLimitNeed() {
        if ($('html').hasClass('wl6')) { setLimit(); } else { removeLimit(); }
    };
